...
class EchoP extends NFNService {
  ...
  override def function(args: Seq[NFNValue], ccnApi: ActorRef): NFNValue = {
    ...
    args match {
      ...
      case Seq(NFNStringValue(pkE), NFNStringValue(pkC)) => {
        permissionChecking(pkE) match {
          case true => {
            // authorized
            lookupKey(pkC) match {
              case Some(skC) => NFNStringValue(skC)  //TOOD: encrypt skC with pkE
              case None => NFNIntValue(1)
            }
          }
          // permission denied
          case false => NFNIntValue(2)
        }
      }
